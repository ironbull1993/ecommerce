<?php
    ...
    if(!empty($full_url) && filter_var($full_url, FILTER_VALIDATE_URL)){
        ...
        if(mysqli_num_rows($sql) > 0){
            ...
            $dt=date('Y-m-d');
            $dt2= mysqli_real_escape_string($conn, $_POST['datee']);
            $sql2 = mysqli_query($conn, "INSERT INTO url (full_url, shorten_url, clicks, date, exp_date) 
                                         VALUES ('{$full_url}', '{$ran_url}', '0', '{$dt}', '{$dt2}')");
            if($sql2){
